...
class CatProfilesRecord {
  static CollectionReference collection =
      FirebaseFirestore.instance.collection("cat_profiles");
...
Future<void> addOrModifyCatProfile(InsertCatProfileQuery catProfile) async {
  final doc = await CatProfilesRecord.collection
      .where('userId', isEqualTo: catProfile.userId)
      .get();
  if (doc.size != 0) {
    await CatProfilesRecord.collection
        .doc(doc.docs.first.id)
        .set(catProfile.toMap());
  } else {
...
List<CatProfilesRecord> createDummyCatProfilesRecord({num count}) {
  return Iterable<int>.generate(10).toList().map((e) => CatProfilesRecord(
      id: "",
      breed: "Breed",
      age: 10,
